        ...
        switch ( subtree.title ) {
            ...
            case "declaration":
                if(parse_tree_helper.is_function_declaration(subtree)){
                    namespace_helper.handle_function_declaration(subtree, namespace);
                }
                else if(parse_tree_helper.is_typedef_declaration(subtree)){
                    namespace_helper.handle_typedef_declaration(subtree, namespace);
                    for(var i = 0; i < subtree.children.length; i++){
                        traverse(subtree.children[i], scopes, namespace);
                    }
                }
                else if(parse_tree_helper.is_record_declaration(subtree)){
                }
                else if(parse_tree_helper.is_variable_declaration(subtree)){
                }
                break;
